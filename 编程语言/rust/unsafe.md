...
- `#[repr(C)]`，C 的表型。
- `#[repr(<primitive>)]`，如 `#[repr(u8)]`，只能用于枚举，对于无字段的枚举，将其尺寸和对齐量设置成给给定表型同名原生类型的值。这可以用于限定枚举中值的数量，比如 `u8` 表形的无字段枚举只能有 `0~255` 的值。对于有字段的枚举，会是一个 `#[repr(C)]` 的联合体，联合体的每个字段都是
- `#[repr(transparent)]`

另外可以使用 `#[repr(packed(val))]` 来修改对齐量，如果未指定表形则修改默认表形的。
