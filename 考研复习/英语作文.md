...
`The picture appears to offer a thought-provoking glimpse into getting comprehensive development. As can be seen, there are two girls contemplating a poster about school lectures. The girl on the left believes that the lectures not related to her major are useless. By contrast, the other girl feels`

$$
\begin{align}
&AQ=Q \Uplambda \implies A=Q \Uplambda Q^T \implies X^TAX = X^TQ \Uplambda Q^TX \implies (Q^TX)^T \Uplambda (Q^TX) \\
&y=Q^TX \implies |\Uplambda y^Ty| < |y^Ty| \implies |\lambda_{1}y^2_{1}+\lambda_{2}y^2_{2}| < |y_{1}^2+y_{2}^2| \implies |\lambda_{1}|,|\lambda_{2}| < 1
\end{align}
$$

$$
\left ( \begin{matrix}X^T_{1} & X_{2}^T
 \end{matrix}\right ) \left ( \begin{matrix}A+E & O \\
O & E-A
 \end{matrix}\right ) \left ( \begin{matrix}X_{1} \\
X_{2}
 \end{matrix}\right ) = X_{1}^T(A+E)X_{1}+X^T_{2}(E-A)X_{2} > 0
$$
