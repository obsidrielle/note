...
记 $U=X-Y$
1. $U \leq 0$ 时
![[Pasted image 20241126122530.png]]
$$
F_{U}(u) = \iint_{x-y \leq u} e^{-(x+y)}d\sigma = \int^{+\infty}_{0}dx \int^{\infty}_{x-u} e^{-(x+y)} dy = \dfrac{e^u}{2}
$$
2. $U \geq 0$ 时
![[Pasted image 20241126122542.png]]
$$
F_{U}(u) = \int^{\infty}_{0}dy \int^{y+u}_{0}e^{-(x+y)}dx=1-\dfrac{e^{-u}}{2} \implies f_{U}(u) = \dfrac{e^{-u}}{2}
$$
则 $Z=|U|$
$$
F_{Z}(z)=P\left\{ |U| \leq z \right\} = P\left\{ -z\leq U \leq z \right\}=\int^{0}_{-z}\dfrac{e^{u}}{2}du + \int^{z}_{0}\dfrac{e^{-u}}{2}=1-e^{-z}  
$$
则 $f_{Z}(z)=e^{-z}$, 即 $Z \sim E(1)$.
