...
设 $u=\dfrac{t}{\theta}$
$$
\dfrac{dF}{dt}=\dfrac{dF}{du}\times \dfrac{du}{dt} = e^{-u^m} m u^{m-1} \dfrac{1}{\theta} = \dfrac{m}{\theta^m} t^{m-1} e^{-\left( \dfrac{t}{\theta} \right)^m }
$$

设 $L(\theta)=\prod \limits_{i=1}^nF'(t_{i})$, 求 $\ln L(\theta)$ 的驻点.
